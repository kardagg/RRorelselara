...
#Testa funtionen
#kdDrawSagittalBodySymmetric(c(0,0,0,0,0,0,0,0),70,180, fixCenterOfMass = TRUE)
  ...
  fluidRow(
    column(4,
           ...
           sliderInput(inputId = "vinkelPelvis", "Bäcken(°)", -180, 
                       180, value = 0, width="100%"),
           sliderInput(inputId = "vinkelUpperArm", "Överarm(°)", -180, 
                       180, value = 0, width="100%"),
           sliderInput(inputId = "vinkelLowerArm", "Underarm(°)", -180, 
                       180, value = 0, width="100%"),
           sliderInput(inputId = "vinkelThigh", "Lår(°)", -180, 
                       180, value = 0, width="100%"),
           sliderInput(inputId = "vinkelShank", "Underben(°)", -180, 
                       180, value = 0, width="100%"),
           sliderInput(inputId = "vinkelFot", "Fot(°)", -180, 
                       180, value = 0, width="100%"),
           fluidRow(
...
server = function(input, output, session) {
  output$sagittalMan = renderPlot({
    kdDrawSagittalBodySymmetric(c(input$vinkelFot,input$vinkelShank,input$vinkelThigh,
                            input$vinkelPelvis,input$vinkelTrunk,input$vinkelHead,
                            input$vinkelUpperArm,input$vinkelLowerArm),
